        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(AttrList, _) in spuSaleAttrList" :key="AttrList.id">
                <dt class="title">选择{{ AttrList.saleAttrName }}</dt>
                <dd changepirce="0" :class="{ active: value.isChecked == 1 }"
                  v-for="(value, _) in AttrList.spuSaleAttrValueList" :key="value.id"
                  @click="changeActive(value, AttrList.spuSaleAttrValueList)">{{
...
export default {
  ...
  methods: {
    ...
    changeSkuNum() {
      // console.log(this.skuNum * 1)
      if (isNaN(this.skuNum * 1) || (this.skuNum <= 0)) {
        this.skuNum = 1
        alert('请输入合法数值（仅正整数）！')
      }
    ...
    async addShopCart() {
      try {
        //调用异步函数，异步函数那边会返回一个promise给调用者，异步函数那边也可以自己加工promise
