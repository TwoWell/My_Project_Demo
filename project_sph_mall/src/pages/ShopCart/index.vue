<template>
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, _) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cart.skuNum = cart.skuNum < 1 ? 1 : cart.skuNum" minnum="1"
              class="itxt" @change="handler('change', $event.target.value * 1, cart)">
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach(cart => {
        if (cart.isChecked == 1) {
          sum = sum + cart.skuPrice * cart.skuNum
        }
      });
      return sum
    },
  ...
  methods: {
    ...
    async handler(type, disNum, cart) {
      ...
      try {
        ...
        // console.log({ skuID: cart.skuId, skuNum: disNum })
        this.getData()
      } catch (error) {
